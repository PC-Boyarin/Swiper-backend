...
const getUser = async (req, res) => {
    try {
        const { id } = req.user;
        const user = await pool.query('SELECT id, username, image_icon FROM users WHERE id = $1', [id]);
...
const updateUser = async (req, res) => {
    try {
        ...
        const user = await pool.query('UPDATE users SET username= $1, image_icon = $2 WHERE id = $3 RETURNING *', [username, image_icon, id]);
        return res.status(200).json(user.rows[0]);
    }
    catch (err) {
